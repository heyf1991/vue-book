...
// 读文件
function read(cb, type) {
  let jUrl = "./mock/book.json";
  if (type === "cart") jUrl = "./mock/cart.json";
  fs.readFile(jUrl, "utf8", function(err, data) {
...
//写入文件
function write(data, cb, type) {
  let jUrl = "./mock/book.json";
  if (type === "cart") jUrl = "./mock/cart.json";
  fs.writeFile(jUrl, JSON.stringify(data), cb);
}
        ...
        case "DELETE":
          read(function(books) {
            let book = books.filter(
              item => parseInt(item.bookId) !== parseInt(id)
            );
            write(book, function() {
              res.end(JSON.stringify({})); //删除操作返回一个空对象
            });
        ...
        case "POST":
          ...
          req.on("end", () => {
            ...
            read(function(books) {
              let isExist = false;
              books.forEach(item => {
                if(parseInt(item.bookId) === parseInt(book.bookId)){
                  item.count+=1;
                  book = item
                  isExist=true; //存在
                }
